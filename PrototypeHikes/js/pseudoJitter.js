function createPseudoJitter(data, height, width) {
    const margin = { top: 10, right: 10, bottom: 0, left: 10 };
    const svgWidth = width - margin.left - margin.right;
    const svgHeight = height - margin.top - margin.bottom;
  
    // Clear existing elements before appending new ones
    d3.select(".PseudoJitter").selectAll("*").remove();
  
    // Create SVG container
    const svg = d3.select(".PseudoJitter")
        .append("svg")
    ...
    svg.append("image")
        .attr("xlink:href", "img/jitter.jpg") // Path to your image
        .attr("x", 0) // Positioning the image
        .attr("y", -30) // Positioning the image
        .attr("width", svgWidth )
